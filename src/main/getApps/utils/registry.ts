...
function ProcessUncleanExitError(this: {
...
function getRegExePath() {
  if (process.platform === 'win32') {
    return path.join(ProcessEnv['windir'] as any, 'system32', 'reg.exe');
  } else {
    return "REG";
  }
}
 ...
 *
 */
function RegistryItem(this: {
  __defineGetter__: any
},host, hive, key, name, type, value, arch):void {
...
export function Registry(this: {
   ...
   */
  this.__defineGetter__('parent', function (this: {
    host: any,
    hive: any,
    arch: any
  }) {
    ...
    if (error) {
      ...
      for (let i = 0, l = items.length; i < l; i++) {
        ...
        if (match) {
          ...
          type = match[2].trim();
          value = match[3];
          (result as any[]).push(new RegistryItem(self.host, self.hive, self.key, name, type, value, self.arch));
        }
      }
...
Registry.prototype.keys = function keys(cb) {
  ...
  proc.stdout.on('end', function () {
    ...
    for (let i = 0, l = items.length; i < l; i++) {
      ...
      let match: RegExpExecArray | null = PATH_PATTERN.exec(items[i]);
      let hive: any;
      let key: any;
      ...
      if (match) {
        ...
        key = match[2];
        if (key && (key !== self.key)) {
          (result as any).push(new Registry({
            host: self.host,
            hive: self.hive,
            key: key,
            arch: self.arch
...
Registry.prototype.keyExists = function keyExists(cb) {
  ...
  this.values(function (err, _items) {
    if (err) {
...
Registry.prototype.valueExists = function valueExists(name, cb) {
  ...
  this.get(name, function (err, _item) {
    if (err) {
